...
import UIKit
import Firebase
class SatildiController: UIViewController,UITableViewDataSource,UITableViewDelegate {
        ...
        dbRef.child("Ilanlar").observe(DataEventType.value, with: {(snapshot) in
            if snapshot.childrenCount  > 0 {
                ...
                for ilanlar in snapshot.children.allObjects as![DataSnapshot]{
                    ...
                    if (ilan.satilma_durumu == "1" && ilan.uid == Auth.auth().currentUser?.uid){
